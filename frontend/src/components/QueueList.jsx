...
export const QueueList = ({ queue, loading }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div className="queue-list-header" style={{ display: "grid", gridTemplateColumns: "1fr 100px 80px", gap: 12, padding: "4px 20px", fontSize: "0.6rem", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--on-surface-variant)" }}>
      ...
      {queue.map((job, idx) => {
        ...
        return (
          <div key={job._id} className="queue-item" style={{
            background: isActive ? "var(--surface-container-highest)" : "var(--surface-container-low)",
            borderRadius: "var(--radius-xl)", padding: "clamp(14px, 3vw, 16px) clamp(16px, 4vw, 20px)",
            ...
            <div className="queue-item-grid" style={{ display: "grid", gridTemplateColumns: "1fr 100px 80px", gap: 12, alignItems: "center" }}>
              {/* Student & file */}
              <div style={{ minWidth: 0 }}>
                <div style={{ fontWeight: 700, color: "var(--on-surface)", fontSize: "clamp(0.8rem, 2vw, 0.875rem)", textTransform: "capitalize" }}>
                ...
                {isActive
                  ? <div style={{ fontSize: "clamp(0.65rem, 1.5vw, 0.7rem)", fontWeight: 700, color: "var(--secondary)", display: "flex", alignItems: "center", gap: 4, marginTop: 2 }}>
                      <span className="material-symbols-outlined" style={{ fontSize: 10 }}>sync</span>
                      PRINTING IN PROGRESS
                    </div>
                  : <div style={{ fontSize: "clamp(0.68rem, 1.6vw, 0.72rem)", color: "var(--on-surface-variant)", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {job.originalName}
                    </div>
                }
              </div>
              ...
              {/* Specs */}
              <div>
                <div style={{ fontSize: "clamp(0.78rem, 1.8vw, 0.82rem)", fontWeight: 600, color: "var(--on-surface)" }}>{totalPages} sheets</div>
                <div style={{ fontSize: "clamp(0.58rem, 1.2vw, 0.6rem)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--on-surface-variant)", marginTop: 2 }}>
